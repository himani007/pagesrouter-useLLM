...
import { AUDIO_TRANSCRIPTIONS_API_URL, CHAT_COMPLETIONS_API_URL, EDIT_IMAGE_API_URL, ELVEN_LABS_DEFAULT_MODEL_ID, ELVEN_LABS_DEFAULT_VOICE_ID, EMBEDDINGS_API_URL, IMAGE_GENERATION_API_URL, IMAGE_VARIATIONS_API_URL, dataURLToBlob, dataUrlToExtension, fillPrompt, getTextToSpeechApiUrl, makeErrorResponse, } from "./utils";
...
export class LLMService {
    constructor({ openaiApiKey = "", elvenLabsApiKey = "", fetcher = fetch, templates = {}, debug = false, isAllowed = () => true, actions = [], }) {
    ...
    async handle({ body = {}, request, }) {
        ...
        if (this.actions.length > 0 && !this.actions.includes($action)) {
        ...
        if ($action === "transcribe") {
            return this.transcribe(rest);
        }
        if ($action === "embed") {
            return this.embed(rest);
        }
        if ($action === "speak") {
            return this.speak(rest);
        }
        if ($action === "generateImage") {
            return this.generateImage(rest);
        }
        if ($action === "editImage") {
            return this.editImage(rest);
        }
        if ($action === "imageVariation") {
            return this.imageVariation(rest);
        }
        throw makeErrorResponse(`Action "${$action}" is not supported`, 400);
    }
    async chat(body) {
...
export default function createLLMService(options = {}) {
